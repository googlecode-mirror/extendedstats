...
def load():
    ...
    weapons.execute("SELECT weapon FROM xs_weapons ORDER BY weapon ASC",2)
...
def cmd_weaponstats(userid,args):
    ...
    weapons.execute("SELECT weapon,kills FROM xs_weapons ORDER BY kills DESC LIMIT 1",2)
    top = weapons.fetchone()
    weapons.execute("SELECT weapon,damage FROM xs_weapons ORDER BY damage DESC LIMIT 1",2)
    pain = weapons.fetchone()
    weapons.execute("SELECT weapon,damage FROM xs_weapons ORDER BY damage ASC LIMIT 1",2)
    pacifist = weapons.fetchone()
    if EE:
        weapons.execute("SELECT weapon,bought FROM xs_weapons ORDER BY bought DESC LIMIT 1",2)
...
def weaponstats(userid,choice,popupid):
    ...
    weapons.execute("SELECT kills,damage%s FROM xs_weapons WHERE weapon='%s'" % (',bought' if EE else '',choice),2)
...
def myweaponstats(userid,choice,popupid):
    ...
    weapons.execute("SELECT kill_%s,death_%s,damage_%s%s FROM xs_main WHERE steamid='%s'" % (choice,choice,choice,(',bought_%s' % choice) if EE else '',steamid),2)
    row = weapons.fetchone()
    p.additem('Kills: %s' % row[0])
