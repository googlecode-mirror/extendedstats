            ...
            if distance > data[steamid]['jump_longest']:
                ...
                if dcfg['notify_longestjump'] == '1':
                    ...
                    if dcfg['notify_longestjump_all'] == '1':
                        ...
                        es.msg('He is ranked %s of %s now. Longest jump overall is %.2f meters by %s' % (rank,len(data)-1,rDistance,rName))
                    else:
                        es.tell(ev['userid'],'You just broke your record of your longest jump! Your new record is: %.2f meters!' % (distance*0.01905))
                        es.tell(ev['userid'],'You are now ranked %s of %s. Longest jump overall is %.2f meters by %s' % (rank,len(data)-1,rDistance,rName))
